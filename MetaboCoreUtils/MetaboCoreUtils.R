...
img <- readPNG("drawings/MetaboCoreUtils_drawing.png")
...
hex <- ggplot() +
    ...
    geom_subview(subview = img, x = 1.00, y = 0.70,
                 width = 0.9, height = 1.5) +
    geom_url("www.bioconductor.org", color = col_grey, size = 2) + 
    geom_pkgname("MetaboCoreUtils", y = 1.343, size = 5.46,
                 color = "#000000", family = "Aller") + 
    geom_pkgname("MetaboCoreUtils", y = 1.35, size = 5.4,
                 color = col_grey, family = "Aller") + 
    theme_sticker()
save_sticker(filename = "MetaboCoreUtils.png", hex)
