...
img <- readPNG("drawings/MsStash-drawing.png")
...
hex <- ggplot() +
    ...
    hex_segment2(linewidth = 0, fill = paste0(col_dark, 10), # right
    ...
    hex_segment2(linewidth = 0, fill = paste0(col_dark, 10), # bottom
                 from_radius = 0, to_radius = 0.9,
    ...
    hex_segment2(linewidth = 0, fill = paste0(col_dark, "ce"), # right
                 ...
                 from_angle = 330, to_angle = 30) +
    hex_segment2(linewidth = 0, fill = paste0(col_dark, "80"), # top right
                 from_radius = 0.9, to_radius = 1,
                 from_angle = 30, to_angle = 90) +
    hex_segment2(linewidth = 0, fill = paste0(col_gold, "ff"), # top left
                 from_radius = 0.9, to_radius = 1,
                 from_angle = 90, to_angle = 150) +
    hex_segment2(linewidth = 0, fill = paste0(col_dark, "80"), # left
                 from_radius = 0.9, to_radius = 1,
                 from_angle = 150, to_angle = 210) +
    hex_segment2(linewidth = 0, fill = paste0(col_dark, "ce"), # bottom
                 from_radius = 0.9, to_radius = 1,
                 from_angle = 210, to_angle = 270) +
    hex_segment2(linewidth = 0, fill = paste0(col_dark, "ce"), # bottom
                 from_radius = 0.9, to_radius = 1,
    ...
    geom_url("MsStash", x = 0.5, y = 1.5, color = col_dark, size = 20,
             angle = 0, family = font_text) +
    ## font size for linux: 6.5, macOS
    geom_url("www.bioconductor.org", x = 0.98, y = 0.17,
             color = col_dark, size = 6.5, family = font_text) + 
    theme_sticker()
save_sticker(filename = "MsStash.png", hex)
