...
img <- readPNG("drawings/sherlock.png")
...
hex <- ggplot() +
    ...
    hex_segment2(size = 0, fill = paste0(col_orange, 10),
                 from_radius = 0, to_radius = 0.9,
                 from_angle = 90, to_angle = 150) +
    hex_segment2(size = 0, fill = paste0(col_orange, 15),
                 from_radius = 0, to_radius = 0.9,
                 from_angle = 150, to_angle = 210) +
    hex_segment2(size = 0, fill = paste0(col_brown, 10),
                 from_radius = 0, to_radius = 0.9,
    ...
    hex_segment2(size = 0, fill = paste0(col_yellow, 70),
                 from_radius = 0.9, to_radius = 1,
                 from_angle = 30, to_angle = 90) +
    hex_segment2(size = 0, fill = paste0(col_orange, 60),
                 from_radius = 0.9, to_radius = 1,
                 from_angle = 90, to_angle = 150) +
    hex_segment2(size = 0, fill = paste0(col_orange, 70),
                 from_radius = 0.9, to_radius = 1,
                 from_angle = 150, to_angle = 210) +
    hex_segment2(size = 0, fill = paste0(col_brown, "fc"),
                 from_radius = 0.9, to_radius = 1,
                 from_angle = 210, to_angle = 270) +
    hex_segment2(size = 0, fill = paste0(col_brown, "ee"),
                 from_radius = 0.9, to_radius = 1,
                 from_angle = 270, to_angle = 330) +
    
    geom_subview(subview = img, x = 1.02, y = 0.99,
                 width = 1, height = 1) +
    ## font size for linux: 6.5, macOS
    geom_url("www.bioconductor.org", x = 1.06, y = 0.09,
             color = col_bg, size = 5, family = font_text) + 
    geom_url("MetaboAnnotation", x = 0.235, y = 1.37, family = font_text,
             color = col_brown, size = 14, angle = 0) + 
    theme_sticker()
save_sticker(filename = "MetaboAnnotation.png", hex)
